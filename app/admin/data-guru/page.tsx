...
const jabatanOptions = [
  "Guru Kelas", "Guru Mata Pelajaran", "Wali Kelas",
...
export default function DataGuruPage() {
  ...
  const fetchGuru = async () => {
    try {
      setLoading(true);
      setError(null);
      const res = await fetch(`${BASE_URL}/guru`, { headers: authHeaders() });
  ...
  const handleSimpan = async () => {
    ...
    setSaving(true);
    try {
      const res = await fetch(`${BASE_URL}/guru/${editData!.id_guru}`, {
        method: "PUT",
        headers: authHeaders(),
          ...
          <table className="w-full text-sm border-collapse table-auto">
            <thead>
              <tr className="bg-gray-200 border-b border-gray-200">
                {["No", "Nama Guru", "NIK", "Jabatan", "Jenis Kelamin", "Tanggal Lahir", "No Telp", "Email", "Alamat", "Aksi"].map((h, i) => (
            ...
            <tbody className="divide-y divide-gray-100">
              {filtered.map((guru, index) => (
                <tr key={guru.id_guru} className="hover:bg-blue-50/40 transition-colors">
                  ...
                  <td className="px-3 py-3 text-gray-600 text-xs border-r border-gray-100 whitespace-nowrap">{guru.tanggal_lahir ?? "-"}</td>
        ...
        <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50">
          <div className="bg-white rounded-xl shadow-xl w-full max-w-md p-6 max-h-[90vh] overflow-y-auto">
            <div className="flex items-center justify-between mb-4">
              <h2 className="text-base font-semibold text-gray-800">Edit Data Guru</h2>
